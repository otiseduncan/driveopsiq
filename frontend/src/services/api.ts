...
export class AuthService {
   ...
   */
  private static storeRole(role: string): void {
    try {
  ...
  private static removeRole(): void {
    try {
      localStorage.removeItem('role');
    } catch (error) {
      console.warn('Failed to remove role:', error);
    }
  }
  ...
  /**
   * Store and retrieve current user
   */
  private static storeCurrentUser(user: User): void {
    try {
      localStorage.setItem('currentUser', JSON.stringify(user));
    } catch (error) {
      console.warn('Failed to store current user:', error);
    }
  }
  ...
  private static removeCurrentUser(): void {
    try {
      ...
      console.warn('Failed to remove current user:', error);
    }
  }

  static getStoredCurrentUser(): User | null {
    try {
      const userJson = localStorage.getItem('currentUser');
      return userJson ? JSON.parse(userJson) : null;
    } catch (error) {
      console.warn('Failed to get stored current user:', error);
      return null;
    }
  }
